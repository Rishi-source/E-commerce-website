...
from secondapp.models import Contact_Us, Category, register_table, add_product,cart,Order,balance,wishlist,Review
...
def register(request):
    ...
    if request.method=="POST":
        ...
        reg = register_table(user=usr, contact_number=con,address = add)
        reg.save()
...
def buy_now(request):
    ...
    wallet = balance.objects.get(user=request.user.id)
    for i in items:
        if wallet.wallet_balance >= float(i.product.sale_price) * i.quantity:
            wallet.wallet_balance -= float(i.product.sale_price) * i.quantity
            wallet.save()
            usr = User.objects.get(username=request.user.username)
            ord = Order(cust_id=usr,cart_ids=cart_ids,product_ids=p_ids,status = True)
            ord.save() 
...
def wallet(request):
    ...
    context = {}
    try:
        wallet_id = balance.objects.get(user__id=request.user.id)
    except balance.DoesNotExist:
        wallet_id = balance.objects.create(user=request.user, balance=0)

    if request.method == "POST":
        ba = 0
        ba = request.POST.get("balance3")
        if ba is not None and float(ba) > 0:
            wallet_id.balance += float(ba)
            wallet_id.wallet_balance = float(wallet_id.balance)
            wallet_id.save()
            print(wallet_id.wallet_balance)
            ba = 0              
...
def wish(request):
    context={}
    items = wishlist.objects.filter(user__id=request.user.id)
    context["items"] = items

    if request.user.is_authenticated:
        if request.method=="POST":
            pid = request.POST["pid"]
            is_exist = wishlist.objects.filter(product__id=pid,user__id=request.user.id)
            if len(is_exist)>0:
                ...
                product =get_object_or_404(add_product,id=pid)
                usr = get_object_or_404(User,id=request.user.id)
                c = wishlist(user=usr,product=product)
                c.save()
                context["msz"] = "{} Added in Your Wishlist".format(product.product_name)
                context["cls"] = "alert alert-success"
    else:
        context["status"] = "Please Login First to View Your Wishlist" 
    return render(request,"wishlist.html",context)
def seller_profile(request):
    context = {}
    sellerID = request.GET["sid"]
    usrss = get_object_or_404(User, id=sellerID)
    products = add_product.objects.filter(seller__id=sellerID)
    context["uuuser"] = usrss
...
def OrderDetails(request):
    ...
    if "oid" in request.GET:
        oid = request.GET["oid"]
        ord = get_object_or_404(Order, id=oid)
        context["order"] = ord
    if "pid" in request.GET:
        pid = request.GET["pid"]
        prd = get_object_or_404(add_product, id=pid)
...
def review(request):
    ...
    if request.method=="POST":
        ...
        data = Review(name=name,contact=con,review=rev, product_id=prd)
        data.save()
...
def review_table(request):
    ...
    if "pid" in request.GET:
        id = request.GET["pid"]
        prd = get_object_or_404(add_product, id = id)
        data = get_object_or_404(Review, product_id__id = prd.id)
    context["data"] = data
    return render(request,"single_product.html",context)
